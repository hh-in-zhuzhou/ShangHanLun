...
import com.nakardo.atableview.view.ATableView.PinnedHeaderAdapter;
...
@SuppressLint("NewApi")
public class ATableViewAdapter extends BaseAdapter implements OnScrollListener,
        PinnedHeaderAdapter {
    ...
    private void initialize() {
        ...
        mNeedPinnedHeader = mTableView.needPinnedHeader();
        ...
        if (dataSource != null) {
            ...
            for (int s = 0; s < sections; s++) {
                ...
                // mark header if overridden in delegate, otherwise try to pull
                // title instead.
                int headerHeight = delegate.heightForHeaderInSection(
                        mTableView, s);
                if (headerHeight != ATableViewCell.LayoutParams.UNDEFINED
                        || dataSource.titleForHeaderInSection(mTableView, s) != null
                        || dataSource.spannerbleTitleForHeaderInSection(mTableView, s) != null) {
                    hasHeader = true;
                }
                mHeadersHeight.add(headerHeight);
                mHasHeader.add(hasHeader);

                // mark footer if overridden in delegate, otherwise try to pull
                // title instead.
                int footerHeight = delegate.heightForFooterInSection(
                        mTableView, s);
                ...
                // pull row heights.
                // closes #23, make sure to pull at least the default height for
                // empty rows sections
                int r = mRows.get(s) - 1;
                do {
                    NSIndexPath indexPath = NSIndexPath
                            .indexPathForRowInSection(r, s);
                    sectionRowHeights.add(delegate.heightForRowAtIndexPath(
                            mTableView, indexPath));
                    r--;
                } while (r >= 0);
    ...
    public int getContentHeight() {
        ...
        // TODO we're supporting content height only for plain tables, it's more
        // complicated for grouped style
        ...
        if (mTableView.getStyle() == ATableViewStyle.Plain) {
            ...
            for (int section = 0; section < mHeadersHeight.size(); section++) {
                height += hasHeader(section) ? getHeaderFooterRowHeight(
                        section, false) : 0;
            }

            // footers.
            for (int section = 0; section < mFootersHeight.size(); section++) {
                height += hasFooter(section) ? getHeaderFooterRowHeight(
                        section, true) : 0;
            }

            // MAGIC MAGIC MAGIC. for some reason we've to add the row count to
            // the height.
    ...
    public NSIndexPath getIndexPath(int position) {
        ...
        for (int s = 0; s < sections; s++) {
            ...
            if (position < limit) {
                // offset is given by current pos, accumulated headers, footers
                // and rows.
                int positionWithOffset = position - offset - count;
                return NSIndexPath.indexPathForRowInSection(positionWithOffset,
                        s);
            }
    ...
    private boolean isSingleRow(NSIndexPath indexPath) {
        return indexPath.getRow() == 0
                && mRows.get(indexPath.getSection()) == 1;
    }
    ...
    private int getRowHeight(ATableViewCell cell, NSIndexPath indexPath,
        ...
        // closes #7. it seems Android ~2.2 requires known row height to draw
        // cell background drawable.
        int rowHeight = mRowsHeight.get(indexPath.getSection()).get(
                indexPath.getRow());
        if (rowHeight < 0) {
            // cached for performance, it might have some impact if user changes
            // the text after layout.
            rowHeight = getMeasuredRowHeight(cell, indexPath, true);
        }
        rowHeight = (int) Math
                .ceil(rowHeight * res.getDisplayMetrics().density);

        // add extra height to rows depending on it's style.
        Rect padding = ATableViewCellDrawable.getContentPadding(mTableView,
                backgroundStyle);
    ...
    private ATableViewCellBackgroundStyle getRowBackgroundStyle(
        ...
        if (isSingleRow(indexPath)) {
            ...
            boolean nextSectionHasHeader = section < mRows.size() - 1
                    && !hasHeader(section + 1);

            // closes #19, plain tables should show separator if next section
    ...
    private ATableViewHeaderFooterCell getReusableHeaderFooterCell(
            View convertView, boolean isFooterRow) {
    ...
    public void setupHeaderFooterRowLayout(ATableViewHeaderFooterCell cell,
                                           NSIndexPath indexPath, boolean isFooterRow) {
        ATableViewDataSource dataSource = mTableView.getDataSource();
        int section = indexPath.getSection();
        Resources res = mTableView.getResources();
        ...
        if (isFooterRow) {
            headerText = dataSource
                    .titleForFooterInSection(mTableView, section);
        } else {
            headerText = dataSource
                    .titleForHeaderInSection(mTableView, section);
        }
        textLabel.setText(headerText);
        // added in 2016.1.3 支持属性文本
        SpannableStringBuilder builder = dataSource
                .spannerbleTitleForHeaderInSection(mTableView, section);
        if (builder != null) {
...
//			System.out.println("headerText:" + headerText);
        ...
        if (mTableView.getStyle() == ATableViewStyle.Grouped) {
            ...
            padding.left = padding.right = (int) res
                    .getDimension(R.dimen.atv_grouped_section_header_footer_padding_left_right);

            // if we're on the very first header of the table and it has text,
            // we've to add an extra padding
            // on top of the cell.
            padding.top = (int) res
                    .getDimension(R.dimen.atv_grouped_section_header_padding_top);
            if (!isFooterRow && section == 0 && hasText) {
                padding.top = (int) res
                        .getDimension(R.dimen.atv_grouped_section_header_first_row_padding_top);
            }

            // if we're on the last footer of the table, extra padding applies
            // here as well.
            padding.bottom = (int) res
                    .getDimension(R.dimen.atv_grouped_section_footer_padding_bottom);
            if (isFooterRow && section == mRows.size() - 1) {
                padding.bottom = (int) res
                        .getDimension(R.dimen.atv_grouped_section_footer_last_row_padding_bottom);
            }

            // hide header or footer text if it's null.
            int visibility = headerText != null && headerText.length() > 0 ? View.VISIBLE
                    : View.GONE;
            ...
            // cell.setBackgroundColor(Color.argb(178, 204, 204, 204));
            // header background
            cell.setBackgroundResource(R.drawable.headerborder);

            TextView label = (TextView) cell.findViewById(R.id.textLabel);
            label.setTextColor(Color.BLACK);
            // huanghai
            label.setGravity(Gravity.CENTER_VERTICAL);
        }
        cell.setPadding(padding.left, padding.top, padding.right,
                padding.bottom);

        // setup layout height
        // closes #16, we've to set minHeight for grouped headers & footers as
        // well to make it take effect.
        int rowHeight = getHeaderFooterRowHeight(indexPath.getSection(),
                isFooterRow);
        if (mTableView.getStyle() == ATableViewStyle.Grouped) {
            int minHeight = (int) res
                    .getDimension(R.dimen.atv_grouped_section_header_footer_min_height);
            cell.setMinimumHeight(rowHeight < minHeight ? rowHeight : minHeight);
        }

        ListView.LayoutParams params = new ListView.LayoutParams(
                ListView.LayoutParams.MATCH_PARENT, rowHeight);
    ...
    private void setupRowLayout(ATableViewCell cell, NSIndexPath indexPath,
        ...
        if (mTableView.getStyle() == ATableViewStyle.Grouped) {
            int margin = (int) cell.getResources().getDimension(
                    R.dimen.atv_cell_grouped_margins);
            cell.setPadding(margin, 0, margin, 0);
        }

        // commented because in android 4.4 it makes cell row height too big to
        // error.
        // ListView.LayoutParams params = new ListView.LayoutParams(
        // ListView.LayoutParams.MATCH_PARENT, rowHeight);
        // cell.setLayoutParams(params);
    ...
    private void setupRowBackgroundDrawable(ATableViewCell cell,
                                            NSIndexPath indexPath,
        ...
        if (selectionStyle != ATableViewCellSelectionStyle.None) {
            ...
            int startColor = res
                    .getColor(R.color.atv_cell_selection_style_blue_start);
            int endColor = res
                    .getColor(R.color.atv_cell_selection_style_blue_end);

            if (selectionStyle == ATableViewCellSelectionStyle.Gray) {
                startColor = res
                        .getColor(R.color.atv_cell_selection_style_gray_start);
                endColor = res
                        .getColor(R.color.atv_cell_selection_style_gray_end);
            }

            ShapeDrawable pressed = new ATableViewCellDrawable(mTableView,
                    backgroundStyle, rowHeight, startColor, endColor);
        ...
        ShapeDrawable normal = new ATableViewCellDrawable(mTableView,
                backgroundStyle, rowHeight, color);
        ...
        if (backgroundView == null) {
            throw new RuntimeException(
                    "Cannot find R.id.backgroundView on your cell custom layout, "
    ...
    private void setupRowContentView(ATableViewCell cell, NSIndexPath indexPath) {
        ...
        // set margins accordingly to content view depending on stroke lines
        // thickness.
        View contentView = cell.getContentView();
        Rect padding = ATableViewCellDrawable.getContentPadding(mTableView,
                backgroundStyle);
        contentView.setPadding(padding.left, padding.top, padding.right,
                padding.bottom);
    }

    private void setupRowAccessoryButtonDelegateCallback(ATableViewCell cell,
        ...
        if (accessoryType == ATableViewCellAccessoryType.DisclosureButton) {
            ...
            accessoryView.setOnClickListener(new View.OnClickListener() {
                public void onClick(View v) {
                    ATableViewDelegate delegate = mTableView.getDelegate();
                    delegate.accessoryButtonTappedForRowWithIndexPath(
                            mTableView, indexPath);
                }
            });
        }
    ...
    private int getHeaderFooterStyleCount() {
        ...
        // closes #10. getViewTypeCount() is only called once by the adapter
        // even when notifyDataSetChanged is invoked,
        // so we've to return header & footer styles even we don't have rows to
        // display.
        if (mTableView.getStyle() == ATableViewStyle.Grouped) {
            count = 2;
        }
    ...
    public int getViewTypeCount() {
        ...
        // count must be 1 always, either we don't have any rows, plus the
        // additional count for header & footers.
        ATableViewDataSource dataSource = mTableView.getDataSource();
        if (dataSource instanceof ATableViewDataSourceExt) {
            // TODO should add custom headers & footers to view count here when
            // supported.
            // getViewTypeCount() is called only once, so no effect if this
            // value is changed on runtime by the user.
    ...
    public int getItemViewType(int position) {
        ...
        if (viewTypeCount > 1) {
            if (isHeaderRow(position)
                    && mTableView.getStyle() == ATableViewStyle.Grouped) {
                viewType = viewTypeCount - 2; // additional style for groped
                // headers.
            } else if (isHeaderRow(position) || isFooterRow(position)) {
                viewType = viewTypeCount - 1; // for plain tables, headers and
                // footers share same style.
            } else {
                ATableViewDataSource dataSource = mTableView.getDataSource();
                if (dataSource instanceof ATableViewDataSourceExt) {
                    NSIndexPath indexPath = getIndexPath(position);
                    viewType = ((ATableViewDataSourceExt) dataSource)
                            .styleForRowAtIndexPath(indexPath);
                }
            }
        }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (isHeaderRow || isFooterRow) {
            ATableViewHeaderFooterCell cell = getReusableHeaderFooterCell(
                    convertView, isFooterRow);

            // setup.
            setupHeaderFooterRowLayout(cell, indexPath, isFooterRow);
            // cell.setAlpha((float) 0.8);
            convertView = cell;
        } else {
            ...
            setupRowBackgroundDrawable(cell, indexPath, backgroundStyle,
                    rowHeight);
            setupRowContentView(cell, indexPath);
            setupRowAccessoryButtonDelegateCallback(cell, indexPath);

            // notify delegate we're about drawing the cell, so it can make
            // changes to layout before drawing.
            ATableViewDelegate delegate = mTableView.getDelegate();
            delegate.willDisplayCellForRowAtIndexPath(mTableView, cell,
                    indexPath);
    ...
    public boolean isEnabled(int position) {
        // TODO disable sounds for header and footer rows, should be changed
        // when supporting clicks on those views.
    ...
    public void onScroll(AbsListView view, int firstVisibleItem,
        ...
        if (mNeedPinnedHeader && view instanceof ATableView) {
            ((ATableView) view).controlPinnedHeader(firstVisibleItem);

            // there make header transparent behind in the pinned header.
            // if (isHeaderRow(firstVisibleItem)) {
            // View v = view.getChildAt(0);
            // if (v != null && v.getBottom() <=
            // mTableView.mHeaderView.getHeight()){
            // v.setVisibility(View.INVISIBLE);
            // }
            // }
            // if (isHeaderRow(firstVisibleItem + 1)) {
            // View v = view.getChildAt(1);
            // if (v != null && v.getTop() > 0)
            // v.setVisibility(View.VISIBLE);
            // }
            // if (isHeaderRow(firstVisibleItem + visibleItemCount - 1)) {
    ...
    public int getPinnedHeaderState(int position) {
        if (getCount() == 0 || position < 0
                || mTableView.mHeaderView.getVisibility() == View.GONE) {
    ...
    public boolean isMove(int pos) {
        if (isHeaderRow(pos + 1)) {
            return true;
        }
        return false;
    }
